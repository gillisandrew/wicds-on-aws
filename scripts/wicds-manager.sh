...
export TERM=dumb ## Fix for systemd exec
...
help() {
  ...
  then
    ...
    cat <<HEREDOC
           _         _
          (_)       | |
 __      ___  ___ __| |___   _ __ ___   __ _ _ __   __ _  __ _  ___ _ __
 \ \ /\ / / |/ __/ _` / __| | '_ ` _ \ / _` | '_ \ / _` |/ _` |/ _ \ '__|
  \ V  V /| | (_| (_| \__ \ | | | | | | (_| | | | | (_| | (_| |  __/ |
   \_/\_/ |_|\___\__,_|___/ |_| |_| |_|\__,_|_| |_|\__,_|\__, |\___|_|
                                                          __/ |
                                                         |___/

World in Conflict dedicated server management CLI.
...
#
###############################################################################
describe "boot" <<HEREDOC
Usage:
  ${_ME} boot

Description:
  Boot windows services.
HEREDOC
boot() {
    export WINEDEBUG=-all
    printf "Starting windows services...\\n"
    /opt/wine-stable/bin/wineserver -p &
    cd "${WICDS_INSTALL_DIRECTORY}"
    wine wic_ds.exe &
    pid=$!
    sleep 10
    kill -TERM $pid
...
describe "start" <<HEREDOC
Usage:
  ${_ME} start <instance-name>

Description:
  Start dedicated server instance with supplied config name.
HEREDOC
start() {
  local _configdir="${WICDS_CONFIG_DIR}"
  if [ -z "${1}" ]; then
    _exit_1 printf "Missing required parameter: %s\\n" "instance-name"
  else
    local _instance="${1}"
    printf "Getting wicds lock...\\n"
    exec 100>"${WICDS_INSTALL_DIRECTORY}/.lock" || _exit_1 printf "Unable to lock installation.\\n"
    flock -w 30 100  || _exit_1 printf "Unable to lock installation.\\n"
    printf "Got wicds lock.\\n"
    printf "Preparing instance config files... %s -> %s\\n" "${_configdir}/${_instance}/" "${WICDS_INSTALL_DIRECTORY}/"
    cp -f "${_configdir}/${_instance}/"* "${WICDS_INSTALL_DIRECTORY}"
    printf "Instance config ready.\\n"
    cd "${WICDS_INSTALL_DIRECTORY}"
    printf "Starting instance...\\n"
    WINEDEBUG=-all wine wic_ds.exe -instance "${_instance}" &
    pid=$!
    echo $pid > "${WICDS_PID_DIR}/${_instance}.pid"
    printf "Instance started. PID %s\\n" "$pid"
    printf "Waiting 2s to ensure the instance is bootstrapped.\\n"
    sleep 2
    flock -u 100 || _exit_1 printf "Unlock failed.\\n"
...
Description:
  Install World in Conflict: Dedicated Server
HEREDOC
install() {
  local _arguments=()
  local _srcdir="/usr/local/src/wicds"
  ...
  if [ -f "${HOME}/.wicds-installed" ]; then
    printf "Dedicated server already installed.\\n"
  else
    . /etc/wicds/environment
    i=0
    ...
    xvfbpid=$!
    printf "Display started PID %s.\\n" "${xvfbpid}"

    printf "Starting install step 1...\\n"
    cp "${WICDS_CONFIG_DIR}/install-1.iss" "${WICDS_CONFIG_DIR}/install-2.iss" "${HOME}/.wine/drive_c"
    wine "${_srcdir}/wic_server_1010.exe"  /s /f1"C:\install-1.iss" &
    sleep 20
    printf "Sending keystrokes...\\n"
    xdotool key Tab Tab a Return
    wait %%
    printf "Install step 1 complete.\\n"
    printf "Starting install step 2.\\n"
    wine "${_srcdir}/wic_server_update_1010_to_1011.exe"  /s /f1"C:\install-2.iss" &
    sleep 20
    printf "Sending keystrokes...\\n"
    xdotool key Tab Tab a Return
    wait %%
    ...
    printf "Applying MP fix...\\n"
    cp "${_srcdir}/d3dx9_33.dll" "${WICDS_INSTALL_DIRECTORY}"
    printf "MP fix applied.\\n"
    printf "Installing extra maps...\\n"
    cp -r "${_srcdir}/maps" "${WICDS_INSTALL_DIRECTORY}/"
    printf "Extra maps installed.\\n"

    printf "Adding installed flag...\\n"
    touch "$HOME/.wicds-installed"
    printf "Flag added.\\n"

    printf "Killing the display...\\n"
    kill $xvfbpid || true
    printf "Display killed.\\n"
