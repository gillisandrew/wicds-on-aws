...
# Scrapes the Wine source code for versions of mono to download for a given version of Wine
...
if [ -z "${WINE_VER}" ]; then
    echo "Please specify the version of wine that requires mono installers"
    echo "e.g."
    echo "  $0 5.0.1"
    exit 1
fi
